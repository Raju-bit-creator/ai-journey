...
The Next.js app also has its own copy of the agent and RAG logic, under `src/lib/agent/` and `src/lib/rag/` — full duplicates of the corresponding `llm/` code, not imports from it. This exists so the web app's `/assistant` page can call this logic directly from a Server Action, since `llm/` and the root app are separate npm projects with no shared module boundary. `src/lib/agent/` also has one extra tool (`classify_iris`, calling the Python service) that the standalone `llm/` version doesn't have.
